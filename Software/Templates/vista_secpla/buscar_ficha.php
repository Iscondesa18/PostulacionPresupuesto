...
<html lang="en">
  ...
  <body  class='fondo'>
    <?php
        ...
        if (!empty($_REQUEST['direccion'])) {
            $buscar_direccion= $_REQUEST['direccion'];
        }
  ...
  <h1 style="text-align: center;">Fichas</h1>
    ...
    <?php
      ...
      WHERE ficha.id_direccion = $buscar_direccion AND estado_ficha.id_estado_ficha!= 1 and estado_ficha.id_estado_ficha!=5
      ...
      <select id="buscar_direccion" class="custom-select"   name="buscar_direccion" >
              ...
              <?php
                ...
                while ($valores = mysqli_fetch_array($query)) {
                    if ($dire == $valores['id_direccion']) {
                         echo '<option value="'.$valores['id_direccion'].'" selected>'.$valores['desc_direccion'].'</option>';
                    }else {
  ...
  <tbody>
    ...
    <?php
      include "../../modelo/conexion_bd.php";
      $sql = $conexion->query("SELECT FORMAT(SUM(financiamiento_solicitado), 'N') as total 
                              FROM `ficha` INNER JOIN direccion ON ficha.id_direccion=direccion.id_direccion 
                              WHERE ficha.id_direccion = $buscar_direccion 
                              AND ficha.id_estado_ficha!= 1 
                              AND ficha.id_estado_ficha!=5
     ");
?>
    <?php
    while($datos = $sql ->fetch_object()){ ?>
